...
import { fetchMediaForScenes } from "@/lib/video/media-fetcher";
  ...
  try {
    ...
    if (needsImages) {
      ...
      scenes = scenes.map((s, idx) => {
        if (!s.needsImage) {
          return { ...s, imageUrl: null, imageSource: null };
        }
        if (imageAssignments.has(idx)) {
          return {
            ...s,
            imageUrl: imageAssignments.get(idx),
            imageSource: "upload",
          };
        }
        // Try stock match
        const stockIdx = unmatchedScenes.findIndex(({ index }) => index === idx);
        if (stockIdx >= 0 && stockScenes[stockIdx]) {
          const stockScene = stockScenes[stockIdx];
          return {
            ...s,
            imageUrl: stockScene.background || stockScene.imageUrl || null,
            imageSource: "stock",
          };
        }
        return { ...s, imageUrl: null, imageSource: null };
      ...
      // Text-only — strip any images
      scenes = scenes.map((s) => ({ ...s, imageUrl: null, imageSource: null }));
    }
