...
import BrandKitHistory from "./BrandKitHistory";
import { Palette, Clock, Sparkles } from "lucide-react";

const TABS = [
  { id: "generate", label: "Generate", icon: Sparkles },
  { id: "history", label: "My Brand Kits", icon: Clock },
];
...
export default function BrandKitWorkspace({
  ...
  const handleGenerated = useCallback((data, usage) => {
    ...
    setSavedId(null);
    if (usage?.used !== undefined) setTodayUsage(usage.used);
  ...
  const handleLoadFromHistory = useCallback((item) => {
    setPrefillData({
      ...item.input_data,
      _loadedKit: item.kit_data,
      _loadedId: item.id,
    });
    setGeneratedData(item.kit_data);
    setSavedId(item.id);
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500 hidden sm:block">
                  Complete brand identity in 60 seconds
                </p>
              </div>
            </div>
        ...
        <div className="bg-amber-50 border-b border-amber-200 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto py-3 flex items-center justify-between gap-4">
            <p className="text-sm text-amber-800">
              You've reached your daily limit of {dailyLimit} brand kit on the
              Free plan.
            </p>
            <a
              href="/pricing"
        ...
        <div className="bg-[#075a01]/5 border-b border-[#075a01]/20 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto py-3 flex items-center justify-between gap-4">
            <p className="text-sm text-[#075a01] font-medium">
              Template loaded. Edit your inputs and generate a new brand kit.
            </p>
            <button
              ...
              {generatedData ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-200 h-full min-h-[400px] flex flex-col items-center justify-center p-8 text-center">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#075a01]/10 to-[#0a8f01]/10 flex items-center justify-center mb-4">
                    <Palette className="w-7 h-7 text-[#075a01]" />
                  </div>
                  <h3 className="text-base font-semibold text-gray-900 mb-2">
                    Your brand kit will appear here
                  </h3>
                  <p className="text-sm text-gray-500 max-w-xs">
                    Fill in the form to generate a complete brand identity —
                    colors, typography, logos, and templates.
                  </p>
                </div>
              )}
            </div>
