...
const STORAGE_KEY = "fd_content_calendar_repurposing_v1";

const TONES = ["Match original", "Professional", "Casual", "Bold", "Friendly", "Witty", "Educational"];
...
export default function RepurposingForm({
  onGenerated,
  isGenerating,
  setIsGenerating,
  atLimit,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.source_content.trim() || form.source_content.length < 100) {
      setError("Paste at least 100 characters of source content.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/ai-content-calendar/generate", {
        ...
        body: JSON.stringify({ ...form, mode: "repurposing" }),
      });
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <h2 className="text-base font-bold text-gray-900">Repurposing Mode</h2>
        <button
          type="button"
          onClick={clearForm}
      ...
      <div>
        <label className={labelClass}>
          ...
          <span className="text-gray-400 font-normal">
            ({form.source_content.length} chars, min 100)
          </span>
        </label>
        <textarea
          value={form.source_content}
          onChange={(e) => update("source_content", e.target.value)}
          placeholder="Paste your blog post, video transcript, article, or any content you want to repurpose..."
          rows={8}
          className={inputClass}
        />
      </div>
      ...
      {(!atLimit || userIsPro) && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold py-3 rounded-xl hover:opacity-90 disabled:opacity-60 transition-all text-sm"
        >
          {isGenerating ? (
            <>
